...
Второе — кэш. .htaccess отдаёт картинки с кэшем на ГОД, поэтому заменить файл на
месте недостаточно: вернувшийся посетитель год видел бы старый. Обычный приём —
дописать ?v= с хэшем содержимого, как сделано для CSS и JS
(tools/bump_asset_versions.py), — здесь обходится слишком дорого: ссылка на логотип
вшита в 95 131 страницу и в оба файла стилей, то есть ради картинки в 11 КБ
пришлось бы переписать весь сайт, а следом ещё раз — чтобы пересчитать ключи самих
стилей. Вместо этого логотипу в .htaccess выдано исключение из годового кэша:
ревалидация раз в сутки, при неизменном файле сервер отвечает 304 без тела.

Ключи всё же нужны? Тогда --stamp-refs: проставит ?v= по хэшу везде, кроме адреса
в разметке schema.org (там нужен постоянный адрес картинки организации), после чего
надо прогнать tools/bump_asset_versions.py.

Скрипт идемпотентен: повторный прогон пересчитывает тот же хэш и ничего не меняет.

Запуск:  python3 tools/clean_logo.py [--dry] [--stamp-refs]
"""
import argparse
import glob
import hashlib
...
def main():
    ...
    ap.add_argument('--stamp-refs', action='store_true',
                    help='дописать ?v= по хэшу всем ссылкам на логотип (95 131 файл)')
    args = ap.parse_args()
    ...
    if not args.stamp_refs:
        print('ссылки не трогаю: кэш логотипа ограничен правилом в .htaccess.')
        print('нужны ключи ?v= — запустите с --stamp-refs')
        return
