...
(function(){
  ...
  var pfx=sub?'../':'';
  var DATA_URL=pfx+'assets/podbor-data.json?v=ef55c80c';
  ...
  // База подбора — 380КБ JSON, её разбор занимает главный поток. Грузим её при
  // приближении к калькулятору, а не при загрузке страницы. Если калькулятор и так
  // виден сразу (страница /podbor), наблюдатель сработает тут же — поведение прежнее.
  // Запас 700px означает «почти доскроллили», так что к моменту показа база уже на месте.
  // Раньше ленивый путь включался только при data-compact=1, то есть на одной главной:
  // 8 страниц каталога тянули и разбирали базу сразу, хотя калькулятор лежит у них
  // в середине страницы и виден не сразу.
...
/* Байт-маркер: lftp mirror --ignore-time сравнивает файлы ПО РАЗМЕРУ. Минифицированный
   podbor.js совпал по длине с боевой версией при разном содержимом — правка молча не
   заливалась, а деплой отчитывался успехом. Менять цифру при каждой правке этого файла,
   если размер снова сойдётся с продом (проверяется tools/check_prod.py). */
var ZR_PODBOR_BUILD=2;
