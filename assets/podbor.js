...
(function(){
  ...
  // Ленивая загрузка базы — только для компактного виджета на главной (data-compact=1).
  // На остальных страницах база грузится сразу, при открытии страницы: так калькулятор
  // готов к работе в тот же момент, когда до него доскроллили, без паузы на загрузку
  // 380КБ. Ленивый путь пробовали включить на всех страницах ради скорости, но по
  // просьбе заказчика вернули прежнее поведение: важнее, чтобы таблица подбора была
  // заполнена сразу, чем выигрыш в замере скорости на каталожных страницах.
